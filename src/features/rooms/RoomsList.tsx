      ...
      {Object.entries(roomsByFloor).sort(([a], [b]) => Number(a) - Number(b)).map(([floor, floorRooms]) => (
        <Paper key={floor} elevation={2} style={{ margin: '16px 0', padding: '16px' }}>
          ...
          <Grid container spacing={2}>
            {floorRooms.sort((a,b) => a.number - b.number).map((room) => {
              ...
              return (
                <Grid item key={room.id} xs={12} sm={6} md={4} lg={2.4}>
                  ...
                  >
                    ...
                    <Typography variant="body2">{room.designation}</Typography>
                    <Typography variant="caption">{translateStatus(room.status)}</Typography>
                    <Typography variant="caption" display="block" style={{ height: '20px'}}>
